...
module.exports = grammar({
  ...
  rules: {
    ...
    // sosl_limit: $=> null,
    sosl_returning_clause: ($) =>
      seq(caseInsensitive("returning"), commaSep1($.sosl_entity_definition)),

    sosl_entity_definition: ($) => seq(field("sobject", $.identifier)),

    _query_value_reference: ($) => choice(seq(":", $.expression), $._literal),
    ...
    from_clause: ($) =>
      seq(caseInsensitive("FROM"), commaSep1($.from_clause_source)),
    ...
    where_clause: ($) =>
      seq(caseInsensitive("WHERE"), $._field_expression_wrapper),
    ...
    field_expression: ($) =>
      choice(
        seq(
          ...
          field(
            "operator",
            choice("=", "!=", caseInsensitive("in"), caseInsensitive("like"))
          ),
    ...
    parenthesized_field_expression: ($) =>
      seq("(", $._field_expression_wrapper, ")"),
    ...
    complex_field_expression: ($) =>
      prec.left(
        ...
        seq(
          ...
          field(
            "operator",
            choice(caseInsensitive("OR"), caseInsensitive("AND"))
          ),
    ...
    where_condition: ($) =>
      choice($.field_expression, seq("(", $.field_expression)),
    ...
    typeof_clause_else_branch: ($) =>
      seq(caseInsensitive("else"), commaSep1($.queried_value)),

    typeof_clause_branch: ($) =>
      seq(
            ...
            field(
              ...
              optional(
                seq(
                  ...
                  choice(caseInsensitive("first"), caseInsensitive("last"))
                )
              )
            )
          )
    ...
    limit_clause: ($) =>
      seq(caseInsensitive("limit"), field("limit", $._query_value_reference)),

    offset_clause: ($) =>
      seq(caseInsensitive("offset"), field("offset", $._query_value_reference)),

    //TODO
    group_by_clause: ($) =>
      seq(caseInsensitive("GROUP"), caseInsensitive("BY")),

    //TODO
    having_clause: ($) => seq(caseInsensitive("having")),
    ...
    dml_insert_statement: ($) =>
      seq(caseInsensitive("insert"), field("value", $.primary_expression), ";"),

    dml_update_statement: ($) =>
      seq(caseInsensitive("update"), field("value", $.primary_expression), ";"),
    ...
    dml_delete_statement: ($) =>
      seq(caseInsensitive("delete"), field("value", $.expression), ";"),

    dml_undelete_statemetn: ($) =>
      seq(caseInsensitive("undelete"), field("value", $.expression), ";"),
    ...
    default_switch_statement_group: ($) =>
      prec.left(
        seq(caseInsensitive("when"), caseInsensitive("default"), $.block)
      ),
    ...
    break_statement: ($) =>
      seq(caseInsensitive("break"), optional($.identifier), ";"),

    continue_statement: ($) =>
      seq(caseInsensitive("continue"), optional($.identifier), ";"),

    return_statement: ($) =>
      seq(caseInsensitive("return"), optional($.expression), ";"),
    ...
    if_statement: ($) =>
      prec.right(
        seq(
          ...
          optional(
            seq(caseInsensitive("else"), field("alternative", $.statement))
          )
        )
      ),
    ...
    inheritance_modifier: ($) =>
      choice(caseInsensitive("virtual"), caseInsensitive("abstract")),

    modifiers: ($) =>
      repeat1(
        choice($.access_modifier, $.inheritance_modifier, "static", "final")
      ),
    ...
    static_initializer: ($) => seq(caseInsensitive("static"), $.block),
    ...
    property_declaration: ($) =>
      seq(
        ...
        "{",
        choice(
          seq($.property_getter, optional($.property_setter)),
          seq($.property_setter, optional($.property_getter))
        ),
        "}"
      ),

    property_getter: ($) =>
      seq(optional($.modifiers), caseInsensitive("get"), choice($.block, ";")),

    property_setter: ($) =>
      seq(optional($.modifiers), caseInsensitive("set"), choice($.block, ";")),
    ...
    extends_interfaces: ($) =>
      seq(caseInsensitive("extends"), $.interface_type_list),
    ...
    integral_type: ($) =>
      choice(caseInsensitive("integer"), caseInsensitive("long")),

    floating_point_type: ($) =>
      choice(caseInsensitive("double"), caseInsensitive("decimal")),
